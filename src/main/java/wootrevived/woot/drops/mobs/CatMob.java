...
public class CatMob extends WootFactoryMob<Cat> {
    ...
    public MutableComponent getDisplayName(CompoundTag mobTag, RegistryAccess registryAccess) {
        ...
        VariantUtils.readVariant(mobTag, registryAccess, Registries.CAT_VARIANT).flatMap(Holder::unwrapKey).ifPresent(key -> {
    ...
    public CompoundTag saveTag(CompoundTag mobTag, RegistryAccess registryAccess){
        ...
        VariantUtils.readVariant(mobTag, registryAccess, Registries.CAT_VARIANT).ifPresent(variant -> VariantUtils.writeVariant(tag, variant));
        return tag;
    }
