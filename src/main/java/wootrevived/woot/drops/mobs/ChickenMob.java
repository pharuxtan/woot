...
public class ChickenMob extends WootFactoryMob<Chicken> {
    public ChickenMob(EntityType<Chicken> entityType, Properties properties) {
    ...
    public MutableComponent getDisplayName(CompoundTag mobTag, RegistryAccess registryAccess) {
        ...
        VariantUtils.readVariant(mobTag, registryAccess, Registries.CHICKEN_VARIANT).flatMap(Holder::unwrapKey).ifPresent(key -> {
    ...
    public CompoundTag saveTag(CompoundTag mobTag, RegistryAccess registryAccess){
        ...
        VariantUtils.readVariant(mobTag, registryAccess, Registries.CHICKEN_VARIANT).ifPresent(variant -> VariantUtils.writeVariant(tag, variant));
        return tag;
    }

    public static void register(WootFactoryMobRegistration registration) {
        registration.registerFactoryMob(new ChickenMob(EntityType.CHICKEN, new Properties()));
    }
}
