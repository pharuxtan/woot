...
public class ChickenMob extends WootFactoryMob<Chicken> {
    ...
    public MutableComponent getDisplayName(ValueInput input) {
        ...
        VariantUtils.readVariant(input, Registries.CHICKEN_VARIANT).flatMap(Holder::unwrapKey).ifPresent(key -> {
    ...
    public void saveTag(ValueInput input, ValueOutput output){
        ...
        VariantUtils.readVariant(input, Registries.CHICKEN_VARIANT).ifPresent(variant -> VariantUtils.writeVariant(output, variant));
    }
